...
def get_critic_featdot(
    ...
    if sample:
        dist, _ = actor(observations=batch["observation"],deterministic=True)
        cur_actions = dist.sample(seed=cur_sample_key)

        next_dist, _ = actor(observations=batch["next_observation"], deterministic=True)
        next_actions = next_dist.sample(seed=next_sample_key)
    else:
        cur_actions, _ = actor(observations=batch["observation"], deterministic=True)
        next_actions, _ = actor(observations=batch["next_observation"], deterministic=True)

    _, cur_critic_info = critic(observations=batch["observation"], actions=cur_actions, deterministic = True)
