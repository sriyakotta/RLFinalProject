    ...
    def __call__(self, x: jnp.ndarray, deterministic:bool) -> jnp.ndarray:
        if self.block_type == "mlp":
            ...
            for _ in range(self.num_blocks):
                x = ResidualBlock(self.hidden_dim, dtype=self.dtype)(x, deterministic=deterministic)
...
class SACActor(nn.Module):
    ...
    def __call__(
        ...
        z = self.encoder(observations, deterministic = deterministic)
...
class TQCCritic(nn.Module):
    ...
    def setup(self):
        ...
        self.predictor = QuantileRegressionCritic(num_quantiles=self.num_quantiles)
    ...
    def __call__(
        ...
        z = self.encoder(inputs, deterministic = deterministic)
        q = self.predictor(z)
        return q
