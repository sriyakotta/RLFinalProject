...
from scale_rl.networks.critics import compute_quantile_loss
...
def update_actor(
    ...
    def actor_loss_fn(
        ...
        if critic_use_cdq:
            q1, q2 = critic(observations=batch["observation"], actions=actions, deterministic = True)
            q = jnp.minimum(q1, q2).reshape(-1)  # (n, 1) -> (n, )
        else:
            q = critic(observations=batch["observation"], actions=actions, deterministic=True)
            q = jnp.mean(q, axis=1)  # average over quantiles → shape = (batch_size,)
...
def update_critic_tqc(
    ...
    next_dist = actor(observations=batch["next_observation"],deterministic=True)
    next_actions = next_dist.sample(seed=key)

    target_quantiles = target_critic(
        observations=batch["next_observation"],
        actions=next_actions,
        deterministic=True,
    )

    sorted_quantiles = jnp.sort(target_quantiles, axis=1)
    truncated = sorted_quantiles[:,:-num_quantiles_to_drop]
    target_value = jnp.mean(truncated, axis=-1)

    target_q = batch["reward"] + (gamma**n_step)*(1-batch["terminated"]) * target_value
    ...
    def critic_loss_fn(
        ...
        # compute quant loss
        critic_loss = compute_quantile_loss(pred_q, target_q[:, None]).mean()
        critic_info = {
